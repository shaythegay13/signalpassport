        ...
        <div className="result-grid">
            ...
            {result.bundle && (
              <div className="card" style={{ marginBottom: 0 }}>
                <div className="card-header-bar">
                  <div className="card-header-left">
                    <span className="card-header-icon">📊</span>
                    <span className="card-header-tag">VERIFIED CLAIMS // STORED ARTIFACT</span>
                  </div>
                  <span className="badge badge-success">Payload Verified</span>
                </div>

                <h3 style={{ margin: "0 0 4px 0", fontSize: "1.25rem" }}>Verified Passport Claims</h3>
                ...
                <div className="verdict-card" style={{ marginBottom: "18px" }}>
          ...
          <div className="result-sidebar">
            ...
            <div className="sidebar-card">
              ...
              <p style={{ color: "var(--text-muted)", fontSize: "0.84rem", margin: "0 0 14px 0", lineHeight: 1.5 }}>
                {result.isValid
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "8px", fontSize: "0.82rem", background: "var(--bg)", padding: "12px", borderRadius: "6px", border: "1px solid var(--border)" }}>
                <div style={{ display: "flex", justifyContent: "space-between", color: "var(--text-muted)" }}>
                  <span>Schema Conformance:</span>
                  <span style={{ color: result.steps.find(s => s.step === "schema")?.passed ? "var(--success-text)" : "var(--danger)", fontWeight: 700 }}>
                    {result.steps.find(s => s.step === "schema")?.passed ? "PASS" : "FAIL"}
                  </span>
                </div>
                <div style={{ display: "flex", justifyContent: "space-between", color: "var(--text-muted)" }}>
                  <span>Subject Format:</span>
                  <span style={{ color: result.steps.find(s => s.step === "subject")?.passed ? "var(--success-text)" : "var(--danger)", fontWeight: 700 }}>
                    {result.steps.find(s => s.step === "subject")?.passed ? "PASS" : "FAIL"}
                  </span>
                </div>
                <div style={{ display: "flex", justifyContent: "space-between", color: "var(--text-muted)" }}>
                  <span>Coverage Sanity:</span>
                  <span style={{ color: result.steps.find(s => s.step === "coverage")?.passed ? "var(--success-text)" : "var(--danger)", fontWeight: 700 }}>
                    {result.steps.find(s => s.step === "coverage")?.passed ? "PASS" : "FAIL"}
                  </span>
                </div>
                <div style={{ display: "flex", justifyContent: "space-between", color: "var(--text-muted)" }}>
                  <span>SHA-256 Digest:</span>
                  <span style={{ color: result.steps.find(s => s.step === "integrity")?.passed ? "var(--success-text)" : "var(--danger)", fontWeight: 700 }}>
                    {result.steps.find(s => s.step === "integrity")?.passed ? "MATCH" : "MISMATCH"}
                  </span>
                </div>
              </div>
            </div>
